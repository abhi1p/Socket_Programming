...
from PyQt5.QtWidgets import QApplication, QLineEdit, QPushButton, QVBoxLayout, QWidget, QTextEdit, QLabel
...
class SendReceiveMessage(QWidget):
    ...
    def set_receive_ip(self):
        ...
        if text:
            self.receive_thread.receive_ip = text
            self.receive_ip = text
        else:
            self.receive_thread.receive_ip = "localhost"
            self.receive_ip = "localhost"
        print(self.receive_thread.receive_ip)

    def set_receive_port(self):
        port = self.receive_port_input.text()
        if port:
            self.receive_thread.receive_port = int(port)
            self.receive_port = int(port)
        else:
            self.receive_thread.receive_port = 5000
            self.receive_port = 5000
        print(self.receive_thread.receive_port)
